...
include("admin/plog-admin-functions.php");
...
function get_album_by_name($name) {
	$sqlAlbum = "SELECT * FROM `".TABLE_PREFIX."albums` WHERE name = '".mysql_real_escape_string($name)."'";
...
function list_albums() {
	...
	$sqlCollections = "SELECT * FROM `".TABLE_PREFIX."collections` ORDER BY `name` ASC";
	...
	$sqlAlbum = "SELECT * FROM `".TABLE_PREFIX."albums` ORDER BY `name` ASC";
	$resultAlbum = run_query($sqlAlbum);
	while ($rowAlbum = mysql_fetch_assoc($resultAlbum)){
		...
		$albums[$i] = array(
			...
			"resize_size" => 480,
			"thumb_size" => 240,
			# no acl system either, if the user is logged in, then she can add/change images
			"perms.add" => "true",
			"perms.write" => "true",
			"perms.del_item" => "true",
...
function list_images($albumname) {
	...
	if ($albuminfo) {
		$sqlPictures = "SELECT * FROM `".TABLE_PREFIX."pictures` WHERE parent_album = " . intval($albuminfo['id']); 
		...
		while ($rowAlbum = mysql_fetch_assoc($resultAlbum)){
			...
			$thumbname = "thumbs/" . $rowAlbum['id'] . '-' . basename($rowAlbum["path"]);
